...
type LogCleanupResult struct {
	...
	DeletedDetailCount int64 `json:"deleted_detail_count"`
}
...
func runLogCleanupTask(ctx context.Context, task *model.SystemTask, runnerID string) {
	...
	deletedDetailCount, err := model.DeleteOldLogDetails(ctx, payload.TargetTimestamp, payload.BatchSize)
	if err != nil {
		failSystemTask(task, runnerID, err)
		return
	}
	...
	result := LogCleanupResult{
		...
		DeletedDetailCount: deletedDetailCount,
	}
