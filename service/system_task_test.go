...
func TestRunLogCleanupTaskDeletesOrphanedLogDetailsWithoutChangingLogProgress(t *testing.T) {
	truncate(t)
	require.NoError(t, model.DB.AutoMigrate(&model.LogDetail{}))
	require.NoError(t, model.DB.Exec("DELETE FROM log_details").Error)
	t.Cleanup(func() {
		require.NoError(t, model.DB.Exec("DELETE FROM log_details").Error)
	})
	...
	const targetTimestamp int64 = 50
	require.NoError(t, model.LOG_DB.Create(&model.Log{
		CreatedAt: targetTimestamp,
		RequestId: "new-log",
	}).Error)
	require.NoError(t, model.DB.Create(&[]model.LogDetail{
		{RequestId: "orphan-old-1", CreatedAt: 10},
		{RequestId: "orphan-old-2", CreatedAt: 20},
		{RequestId: "new-detail", CreatedAt: targetTimestamp},
	}).Error)

	payload := LogCleanupPayload{TargetTimestamp: targetTimestamp, BatchSize: 1}
	...
	runLogCleanupTask(context.Background(), claimedTask, "runner-log-detail-cleanup")
	...
	require.NoError(t, common.UnmarshalJsonStr(finishedTask.State, &state))
	assert.Equal(t, int64(0), state.Total)
	...
	assert.Equal(t, int64(2), result.DeletedDetailCount)

	var oldDetailCount int64
	require.NoError(t, model.DB.Model(&model.LogDetail{}).
		Where("created_at < ?", targetTimestamp).
		Count(&oldDetailCount).Error)
	assert.Equal(t, int64(0), oldDetailCount)

	var newDetailCount int64
	require.NoError(t, model.DB.Model(&model.LogDetail{}).
		Where("request_id = ?", "new-detail").
		Count(&newDetailCount).Error)
	assert.Equal(t, int64(1), newDetailCount)

	var newLogCount int64
	require.NoError(t, model.LOG_DB.Model(&model.Log{}).
		Where("request_id = ?", "new-log").
		Count(&newLogCount).Error)
