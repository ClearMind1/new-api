...
const logSettingsSchema = z.object({
  ...
  LogDetailCaptureRequestBody: z.boolean(),
  LogDetailCaptureResponseBody: z.boolean(),
  LogDetailCaptureHeaders: z.boolean(),
  LogDetailRetentionDays: z.number().int().min(0).max(365),
  LogDetailMaxBodySizeKB: z.number().int().min(1).max(1024),
})
      ...
      onSubmit: async (_values, changedFields) => {
        for (const [key, value] of Object.entries(changedFields)) {
          await updateOption.mutateAsync({
            key,
            value: value as LogSettingsFormValues[keyof LogSettingsFormValues],
          })
        }
      },
    })
          ...
          <SettingsControlGroup>
            <div>
              <h4 className='text-sm font-medium'>
                {t('Request Detail Settings')}
              </h4>
              <p className='text-muted-foreground mt-1 text-xs'>
                {t(
                  'Capture request and response details for troubleshooting and auditing.'
                )}
              </p>
            </div>
              ...
              render={({ field }) => (
                <SettingsSwitchItem className='py-2'>
                  <SettingsSwitchContent>
                    ...
                    <FormDescription>
                      {t(
                        'Store request details alongside usage logs for later inspection.'
                      )}
                    </FormDescription>
                  </SettingsSwitchContent>
                  <FormControl>
            ...
            <SettingsControlChildren className='flex flex-col gap-4'>
              <div className='grid gap-x-5 gap-y-2 md:grid-cols-3'>
                <FormField
                  control={form.control}
                  name='LogDetailCaptureRequestBody'
                  render={({ field }) => (
                    <SettingsSwitchItem className='py-2'>
                      <SettingsSwitchContent>
                ...
                <FormField
                  control={form.control}
                  name='LogDetailCaptureResponseBody'
                  render={({ field }) => (
                    <SettingsSwitchItem className='py-2'>
                      <SettingsSwitchContent>
                ...
                <FormField
                  control={form.control}
                  name='LogDetailCaptureHeaders'
                  render={({ field }) => (
                    <SettingsSwitchItem className='py-2'>
                      <SettingsSwitchContent>
                        <FormLabel>{t('Capture request headers')}</FormLabel>
                        <FormDescription>
                          {t('Sensitive headers are excluded.')}
                        </FormDescription>
                      </SettingsSwitchContent>
                      <FormControl>
                        <Switch
              ...
              <div className='grid gap-4 md:grid-cols-2'>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        {t(
                          'Set to 0 to disable automatic cleanup. At least 7 days is recommended.'
                        )}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  control={form.control}
                  name='LogDetailMaxBodySizeKB'
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t('Maximum detail body size (KB)')}
                      </FormLabel>
                      <FormControl>
                        <Input
                          type='number'
                      ...
                      <FormDescription>
                        {t(
                          'Request and response bodies beyond this size are truncated. 64 KB is recommended.'
                        )}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
